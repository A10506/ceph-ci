...
int DaemonServer::init(uint64_t gid, entity_addr_t client_addr)
{
  ...
  int ret = admin_socket->register_command("dump_cluster_state",
                                       "dump_cluster_state",
                                       m_daemon_hook,
...
bool DaemonServer::handle_command(MCommand *m)
{
  ...
  if (prefix == "pg scrub" ||
    ...
    cluster_state.update_image_stats(id, opstat, name);
    cmdctx->reply(0, "");
    return true;
    ...
    if (f) {
      cluster_state.dump_imgsperf(f.get(), what);
      f->flush(cmdctx->odata);
    } else {
      stringstream dumps;
      ss << "dumping: " << what;
      cluster_state.dump_imgsperf(dumps, what);
    ...
    for (const auto& line : lines) {
      ...
      cache_map[it->second] = osd_update_cache;
      ldout(cct, 10) << " update cache_map from osd." << it->second
    ...
    std::map<uint64_t, pool_cache_t> pool_caches;
    cluster_state.with_osdmap([&] (const OSDMap& osdmap) {
        for (std::map<int, cache_stat_t>::const_iterator it = cache_map.begin(); 
             it != cache_map.end(); it++) {
          auto osd = it->first;
...
void DaemonServer::tick() {
  cluster_state.sample_perf_stats();
